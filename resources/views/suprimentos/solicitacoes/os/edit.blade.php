...
{{--        <a href="{{ route('almoxarifado.solicitacao.compras.os.create',$solicitacao->id) }}" class="btn btn-info m-1">--}}
    ...
    <div class="col-md mt-3">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h3 class="w-50 float-left card-title m-0">Solicitação Código : {{ (!empty($solicitacao->codospedido)) ? $solicitacao->codospedido : '' }}</h3>
            </div>
            <div class="card-body">
...
{{--                                <input type="text" id="itens" name="itens" class="form-control" value="{{ $solicitacao->itens }}" readonly>--}}
                            ...
                            <div class="col-md-12 mt-2">
                                ...
                                <textarea class="form-control" id="comentario_estoque" name="comentario_estoque" rows="5">{{ $solicitacao->comentario_estoque }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row" x-data="handler()">
                            <div class="col-md-12 mt-4">
                                <label for="recipient-name-2" class="col-form-label">Saida de Produtos do Estoque ( visao somente do estoque )</label>
...
{{--                                        <input x-model="field.txt1" type="text" name="txt1[]" class="form-control">--}}
                            ...
                            <tfoot>
                            <!-- You can only add if it is open/aberta -->
                            ...
                            @else
                                    @if(count($saida) > 0)
                                        @foreach($saida as $key => $produto)
                                        <tr>
                                            <td>{{ $key }}</td>
                                            <td>
                                                <input  type="text" name="nomeproduto{{ $key }}" class="form-control" value="{{ $produto->nome }}" readonly>
                                            </td>
                                            <td><input  type="text" name="qtdproduto{{$key}}" class="form-control" value="{{ $produto->quantidade }}" readonly></td>
                                            <td></td>
                                        </tr>
                                        @endforeach
                                    @endif
