    ...
    <div class="col-md mt-3">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h3 class="w-50 float-left card-title m-0">Solicitação de Compra Código : {{ $solicitacao_compra->codigo_solicitacao_compra }}</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('almoxarifado.saida.estoque.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="solicitacao_id" value="{{ $solicitacao_compra->id }}">
...
{{--                                            @endforeach--}}
                                ...
                                <div class="col-md-6">
                                    <label for="recipient-name-2" class="col-form-label">Solicitacao:</label>
                                    <textarea class="form-control" id="solicitacao" name="solicitacao" rows="5" readonly> {{ $solicitacao_compra->solicitacao }}</textarea>
                                </div>
                            <div class="col-md-12 mt-2">
                                <label for="recipient-name-2" class="col-form-label">Resposta do Gestor:</label>
                                <textarea class="form-control" id="resposta" name="resposta" rows="5" readonly> {{ $solicitacao_compra->reposta }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row" x-data="handler()">
                            <div class="col-md-12 mt-4">
                                <label for="recipient-name-2" class="col-form-label">Produtos/Itens Solicitados</label>
                            ...
                            @foreach($solicitacao_itens as $key => $item)
                                <tr>
                                    <td>{{ $key }}</td>
                                    <td><input type="text" name="produto{{$key}}" class="form-control" value="{{$item->nome}}" readonly></td>
                                    <td><input type="text" name="quantidade{{$key}}" class="form-control" value="{{$item->quantidade}}" readonly></td>
                                </tr>
                            @endforeach
                            </tbody>
                            <tfoot>
